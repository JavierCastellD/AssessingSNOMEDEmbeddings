...
class EmbeddingPredictor:
    ...
    def __init__(self, embedding_space : dict, batch_size : int, epochs : int, dense_layers : list, 
        ...
        # FAISS
        embedding_values = list(embedding_space.values())
        self.index2sct = list(embedding_space.keys())
        ...
        self.faiss_index = faiss.index_factory(self.embedding_size, "Flat", faiss.METRIC_INNER_PRODUCT)
    ...
    def predict_faiss(self, X_test, X_ids = None, return_top10 : bool = True):
        ...
        # We use FAISS here now
        q_vectors = np.array(prediction).astype(np.float32)
        faiss.normalize_L2(q_vectors)
        
        sim_values, index = self.faiss_index.search(q_vectors, 10)
        
        #similarity_values = cosine_similarity(prediction, space_embeddings)
        
        for i in range(len(index)):
            sim_concepts = index[i]
            sim_zip = [self.index2sct[i_concept] for i_concept in sim_concepts]
